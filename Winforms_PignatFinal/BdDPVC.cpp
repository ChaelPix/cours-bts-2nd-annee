...
bool CBdDPVC::connecter() {
	...
	try {
		con = driver->connect("tcp://10.187.52.4:3306", "laurentr", "laurentr");
		con->setSchema("laurentr_b");
...
CFormule CBdDPVC::makeFormule(int idF) {
	...
	if (res->next())
	{
		...
		std::string type_string = res->getString("type");
		CFormule::Type type;
		type_string == "SOUPLE" ? type = CFormule::Type::SOUPLE : type = CFormule::Type::RIGIDE;

		CFormule formule(type, pvcBase, plastifiant, lubrifiant, dureeMalaxage, dureeRefroidissement);
		return formule;
